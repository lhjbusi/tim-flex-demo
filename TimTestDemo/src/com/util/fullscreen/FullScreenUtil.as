package com.util.fullscreen {
	...
	public class FullScreenUtil {
		...
		public static function exitFullScreen():void {
			...
			if (theCanvas.systemManager.stage.displayState == StageDisplayState.FULL_SCREEN)
				theCanvas.systemManager.stage.displayState = StageDisplayState.NORMAL;
		...
		private static function updateAnchorStates():void {
			for (var key:Object in anchorMap) {
				if (key is UIComponent) {
					...
					if(anchorLeft == -1 && anchorRight == -1 && stretchProportional)
						uic.x = 0;
					if(anchorTop == -1 && anchorBottom == -1 && stretchProportional)
						uic.y = 0;

					if(centerVertically)
						uic.y = Application.application.screen.height / 2 - uic.height / 2;
					if(centerHorizontally)
						uic.x = Application.application.screen.width / 2 - uic.width / 2;
				}
			}
		}
